...
## 2026-07-21 — Add 5 contemporary event files
- Authored `war-in-afghanistan-2001`, `indian-ocean-tsunami-2004`, `iphone-launched`, `tohoku-earthquake-fukushima`, `chatgpt-launched` in `content/events/`. Each has 4 dense-prose sections (2-3 with Wikimedia images), 5-7 named figures (1-2 with portraits), 1-2 hero images, and 5 sources — all URLs verified 200.

## 2026-07-21 — Add 6 more contemporary event files (war and politics)
- Authored `iraq-war-begins` (2003-03-20), `russia-annexes-crimea` (2014-03-18), `russia-invades-ukraine` (2022-02-24), `october-7-attacks-and-gaza-war` (2023-10-07), `fall-of-assad-regime` (2024-12-08), `venezuela-election-crisis-2024` (2024-07-28) in `content/events/`. Each: 4 dense-prose sections (2-3 with Wikimedia images), 5 named figures (2-4 with Wikimedia portraits), 1-2 hero images, and 4-6 sources — all URLs and Wikimedia file pages verified. Added 16 total figure portraits across the six events. No `worldContext`, per the contemporary-events rule.

## 2026-07-21 — Content enrichment pass checkpoint
- Verified all 90 event JSON files + 15 year files + top-level content JSON parse cleanly (no half-edited files left behind after the enrichment pass paused).

## 2026-07-21 — Enrich 20 event files with Wikimedia figure portraits and section images
- Added `imageUrl`/`credit` on ~35 figures (Metternich, Talleyrand, Alexander I, Wellington, Copernicus, Paul III, Tedros, Fauci, JFK, RFK, Khrushchev, Castro, Darwin, Wallace, Huxley, Alexander, Ptolemy I, Socrates, Plato, Xenophon, Peter, Cyrus/Mehmed via section pics, Boabdil, Isabella, Ferdinand II, Cortes, Cuauhtemoc, Moctezuma, Malinche, Constantine, Urban II, Kohl, Gorbachev, Yeltsin, Lin Zexu, Daoguang, Gladstone, Mao, Chiang) across the 20-file bucket.
- Added 40+ section `image` fields (2-3 per event) across all 20 events — sourced from Wikimedia Commons, direct upload URLs verified, license labels copied from the file description pages (Public Domain / CC BY 2.0 / CC BY-SA 3.0 / CC BY-SA 4.0 / CC BY 4.0 / CC BY 2.5).
- Skipped figures with no Wikimedia likeness (Iftikhar al-Dawla, Osiander, Petreius, Giese, Rheticus, Astyages, Croesus, Nabonidus, Perdiccas, Antigonus, Meletus, Anytus, Kravchuk, Shushkevich, Nazarbayev, Talavera, Cisneros, Alvarado, Orestes, Nepos, Zeno, Odoacer, Godfrey, Raymond, Bohemond, Elliot, Palmerston, Pottinger, Zhou, Zhu De, Liu, Song, Van Kerkhove, Ryan, Li Wenliang, Hardenberg, Castlereagh, Julius/Pius popes, del Monte, Borromeo, Charles V, Lactantius, Eusebius, Galerius, Maximinus Daia, Licinius, Schabowski, Jaeger, Krenz, Dobrynin, Arkhipov, Hooker, Lyell, John Murray) — several because searches returned no reliably available Wikimedia portrait file, others (Li Wenliang) because only fair-use en.wiki files exist.
- Build (`npm run build`) passes; no Zod validation errors.

## 2026-07-21 — Enrich 20 event files (second bucket) with Wikimedia figure portraits and section images
- Bucket: founding-of-rome, founding-of-united-nations, fourth-crusade-sack-of-constantinople, genghis-khan-proclaimed, great-pyramid-of-giza, haitian-revolution-begins, handover-of-hong-kong, house-of-wisdom-baghdad, invention-of-writing-sumer, justinian-corpus-juris, luther-95-theses, magna-carta, mansa-musa-hajj, meiji-restoration, ming-dynasty-founded, mongol-sack-of-baghdad, napoleon-crowned-emperor, newton-principia, olmec-san-lorenzo, outbreak-of-wwi.
- Added ~48 figure `imageUrl`/`credit` fields across the 20 events (FDR, Truman, Churchill, Stalin, Trygve Lie, Eleanor Roosevelt; Innocent III, Niketas Choniates; Khufu, Hemiunu; Dessalines, Sonthonax; Jiang Zemin, Charles/Prince of Wales, Chris Patten, Blair, Tung Chee-hwa, Deng; Hunayn ibn Ishaq; Henry Rawlinson; Justinian; Luther, Tetzel, Leo X; King John, William Marshal, Innocent III; Meiji emperor, Yoshinobu, Saigo, Okubo, Kido, Iwakura, Ito; Hongwu, Empress Ma; Napoleon, Pius VII, Josephine, David; Newton, Halley, Wren, Pepys; Franz Ferdinand, Sophie, Franz Joseph, Wilhelm II, Nicholas II; Titus Livy).
- Added 55+ section `image` fields (2-3 per event) sourced from Wikimedia Commons; every direct upload URL and license label verified against the Commons file description page.
- Skipped figures with no historical Wikimedia likeness: Romulus, Remus, Varro (myth); anonymous Sumerian scribes, Schmandt-Besserat, Julius Jordan; the Olmec archaeologists (no Commons portraits); al-Ma'mun, al-Khwarizmi, Banu Musa, al-Kindi, Thabit ibn Qurra (only modern imaginative renderings on Commons); Stephen Langton, Robert Fitzwalter, Percier/Fontaine, Empress Ma-later figures already skipped, Al-Musta'sim, Ibn al-Alqami, Nasir al-Din al-Tusi (modern stamp only), Doquz Khatun, Abu Ishaq al-Sahili, Al-Nasir Muhammad, Al-Umari, Ibn Battuta, Jamukha, Teb Tengri, Merer, Sneferu, Hetepheres, Dutty Boukman, Cecile Fatiman, Boniface of Montferrat, Baldwin IX, Alexios V, Albert of Brandenburg, Frederick the Wise, Xu Da, Toghon Temur, Liu Ji, Hulagu (existing URL 404), Hooke, Gavrilo Princip (only AI-colorised versions).
- Build (`npm run build`) passes with zero Zod errors.

## 2026-07-21 — Timeline: fix overlap, clipping, and missing connectors
- Rewrote label placement in `lib/timeline/highcharts-config.ts` for both the eras timeline (mode 1) and per-era event timeline (mode 2): each point is now assigned one of 4 lanes (near-above, near-below, far-above, far-below) via a per-point `__laneIdx`, and a chart `render` handler manually shifts far-lane labels away from the ribbon inside a `requestAnimationFrame` (so it lands after Highcharts's own layout pass).
- Rewired connectors to redraw from marker to label edge in the parent group's local coordinate space, and force `opacity: 1` on the connector path (Highcharts otherwise applies `opacity: 0` when its own collision heuristic disagrees with our manual placement). Every event now has a visible connector line reaching its ribbon dot.
- Truncated visible event titles with a word-aware ellipsis (24 chars mobile / 34 chars desktop) so labels stay within the fixed-width callout; full title still surfaces in the hover tooltip via the existing `tooltipHTML` formatter (unchanged).
- Reduced label width and font-size on mobile (era timeline: 120px @ 10.5px, eras: 100px @ 10px), sharpened side spacing (55px mobile / 90px desktop) so first/last labels never clip against the container.
- Verified in the browser across all 8 eras at 1440x900 and mobile viewports: Prehistory, Ancient, Classical, Post-classical, Early Modern, Long 19th Century, 20th Century, Contemporary. Tooltip still renders the full hero image + title + summary card on hover.
